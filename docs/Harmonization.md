...
|weak encyption|Incorrect uses of encryption algorithms may result in sensitive data exposure, key leakage, broken authentication, insecure session, and spoofing attacks. There are some encryption or hash algorithms known to be weak and are not suggested for use.
|test|Used for testing purposes.|These observations can be used to test an early warning service for example, without impacting the functionality of the service.|
