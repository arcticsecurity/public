...
|defacement|This type of observation refers to hacktivism, which on a technical level is indicative of suspected compromise.|This host is likely to have been compromised by a third party and very often is used for other criminal activities as well.|
...
|cve|This type of observation identifies a product and a version of software, which contains a specific vulnerability.|These observations do not detail the affected host or service, rather they can be used to identify such services especially if they are not directly exposed to the Internet or refer to client side vulnerabilities.|
